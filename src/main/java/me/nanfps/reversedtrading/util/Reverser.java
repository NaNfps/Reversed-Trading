...
public final class Reverser {
    ...
    public static TradeOffer reverse(TradeOffer it) {
        return it.getSecondBuyItem().isEmpty()
                       ? new TradeOffer(
                               new TradedItem(it.getSellItem().getRegistryEntry(), it.getSellItem().getCount(),
                                       ComponentPredicate.of(it.getSellItem().getComponents())
                               ), it.getSecondBuyItem(), it.getOriginalFirstBuyItem(), it.getUses(), it.getMaxUses(), it.getMerchantExperience(), it.getPriceMultiplier(), it.getDemandBonus())
                       : it;
    }

    public static TradeOfferList reverse(TradeOfferList original, MerchantEntity abstractVillager) {
        if(abstractVillager != null && shouldEnable(abstractVillager) && isNameCorrect(abstractVillager.getName().getString())) {
            return new TradeOfferList(original.stream().map(Reverser::reverse).toList());
        }
